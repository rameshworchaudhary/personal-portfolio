...
// Intelligent fallback responses if Groq API key is missing or fails
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (apiKey && apiKey.trim() !== '' && !apiKey.includes('your_groq_api_key')) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn('Groq API response not OK:', response.status, await response.text());
        }
      } catch (groqErr) {
    ...
    return res.status(500).json({ 
      reply: "Sorry, I encountered an issue processing that question. Please try again!" 
    });
  }
});
